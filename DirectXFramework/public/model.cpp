...
vnModel::vnModel(const WCHAR* folder, const WCHAR* file) : vnObject()
{
	...
	for (int i = 0; i < (int)pModelData->MaterialNum; i++)
	{
		...
		{
			//テクスチャの拡張子
			const WCHAR* ext = wcschr(pMaterialData[i].Texture, '.');
		...
		pMaterials[i].TilingOffset = XMVectorSet(pMaterialData[i].TilingOffset[0], pMaterialData[i].TilingOffset[1], pMaterialData[i].TilingOffset[2], pMaterialData[i].TilingOffset[3]);
	}
...
void vnModel::render()
{
	...
	else
	{
		...
		//World = XMMatrixMultiply(Local, *pParent->getWorld());
	}
	...
	for (int i = 0; i < (int)pModelData->MeshNum; i++)
	{
		...
		XMStoreFloat4(&pMaterials[mid].pConstBuffer->Diffuse, pMaterials[mid].Diffuse);
		XMStoreFloat4(&pMaterials[mid].pConstBuffer->Ambient, pMaterials[mid].Ambient);
		XMStoreFloat4(&pMaterials[mid].pConstBuffer->Specular, pMaterials[mid].Specular);
		XMStoreFloat4(&pMaterials[mid].pConstBuffer->TilingOffset, *getTilingOffset(mid));
		...
		if (lighting)
		{
			...
			{
				//半透明有り・ライティング有り
				vnDirect3D::getCommandList()->SetPipelineState(pPipelineState_T);
			}
			else
			{
				//半透明無し・ライティング有り
		...
		else
		{
			...
			{
				//半透明有り・ライティング無し
				vnDirect3D::getCommandList()->SetPipelineState(pPipelineState_T_NL);
			}
			else
			{
				//半透明無し・ライティング無し
...
#ifdef _DEBUG
	if (aabbCollider.getEnable())
	{
		aabbCollider.debugLineRenderer();
	}
#endif // _DEBUG
...
const WCHAR* vnModel::getTextureName(int id)
{
...
//タイリングとオフセットの設定
void vnModel::setTilingOffset(const XMVECTOR* v, int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return;
	if (id == -1)
	{
		for (int i = 0; i < getMaterialNum(); i++)
		{
			pMaterials[i].TilingOffset = *v;
		}
	}
	else if (id >= 0 && id < getMaterialNum())
	{
		pMaterials[id].TilingOffset = *v;
	}
}

void vnModel::setTilingX(float value, int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return;
	if (id == -1)
	{
		for (int i = 0; i < getMaterialNum(); i++)
		{
			pMaterials[i].TilingOffset = XMVectorSetX(pMaterials[i].TilingOffset, value);
		}
	}
	else if (id >= 0 && id < getMaterialNum())
	{
		pMaterials[id].TilingOffset = XMVectorSetX(pMaterials[id].TilingOffset, value);
	}
}

void vnModel::setTilingY(float value, int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return;
	if (id == -1)
	{
		for (int i = 0; i < getMaterialNum(); i++)
		{
			pMaterials[i].TilingOffset = XMVectorSetY(pMaterials[i].TilingOffset, value);
		}
	}
	else if (id >= 0 && id < getMaterialNum())
	{
		pMaterials[id].TilingOffset = XMVectorSetY(pMaterials[id].TilingOffset, value);
	}
}

void vnModel::setOffsetX(float value, int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return;
	if (id == -1)
	{
...
void vnModel::setOffsetY(float value, int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return;
	if (id == -1)
	{
...
//タイリングとオフセットの取得
const XMVECTOR* vnModel::getTilingOffset(int id)
{
	if (pMaterials == NULL || getMaterialNum() <= 0)return NULL;
	return &pMaterials[id].TilingOffset;
}

AABBCollider& vnModel::getAABBCollider()
{
	return aabbCollider;
}
