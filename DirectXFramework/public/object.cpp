...
ID3D12PipelineState* vnObject::pPipelineState_T = NULL;
ID3D12PipelineState* vnObject::pPipelineState_T_NL = NULL;
...
bool vnObject::initializeCommon()
{
	...
	hr = vnDirect3D::getDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pPipelineState_T));
	pPipelineState_NL->SetName(L"vnObject::pPipelineState_T");
	...
	hr = vnDirect3D::getDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pPipelineState_T_NL));
	pPipelineState_NL->SetName(L"vnObject::pPipelineState_T_NL");
...
void vnObject::terminateCommon()
{
	...
	SAFE_RELEASE(pPipelineState_T);
	SAFE_RELEASE(pPipelineState_T_NL);
}
...
//親オブジェクトの設定
void vnObject::setParent(vnObject* pObj)
{
	pParent = pObj;
}

//親オブジェクトの取得
vnObject* vnObject::getParent()
{
	return pParent;
}
