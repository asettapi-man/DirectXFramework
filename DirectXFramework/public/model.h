...
protected:
	...
	AABBCollider aabbCollider;	//AABBの当たり判定
...
public:
	...
	int getMaterialNum();
	int getMeshNum();
	const WCHAR *getTextureName(int id);
	...
	void setDiffuseR(float value, int id=-1);	//オブジェクトの色設定
	void setDiffuseG(float value, int id=-1);	//オブジェクトの色設定
	void setDiffuseB(float value, int id=-1);	//オブジェクトの色設定
	void setDiffuseA(float value, int id=-1);	//オブジェクトの透明度設定
	...
	//タイリングとオフセットの設定
	void setTilingOffset(const XMVECTOR* v, int id = -1);
	void setTilingX(float value, int id = -1);
	void setTilingY(float value, int id = -1);
	void setOffsetX(float value, int id = -1);
	void setOffsetY(float value, int id = -1);

	//タイリングとオフセットの取得
	const XMVECTOR* getTilingOffset(int id = 0);
