...
vnBillboard::vnBillboard(float width, float height, const WCHAR* texture_file, float left_u, float right_u, float top_v, float bottom_v) : vnObject()
{
	...
	TillingOffset = XMVectorSet(1.0f, 1.0f, 0.0f, 0.0f);
...
void vnBillboard::render()
{
	...
	XMStoreFloat4(&pConstBuffer->TilingOffset, TillingOffset);
...
void vnBillboard::setVertexPosition()
{
	...
	//逆行列を計算（※アフィン変換の回転成分のみのため、転置行列と同じになる）
	mBillboard = XMMatrixTranspose(mBillboard);

	XMVECTOR worldPos;	//ワールド座標の取得
	...
	for (int i = 0; i < 4; i++)
	{
		...
		//ワールド座標を適応
		v[i] = XMVectorAdd(v[i], worldPos);

		//レンダリングされる頂点（vtx）に座標を代入
		vtx[i].x = XMVectorGetX(v[i]);
		vtx[i].y = XMVectorGetY(v[i]);
		vtx[i].z = XMVectorGetZ(v[i]);
	}

	vnDebugDraw::Line(&v[0], &v[1], 0xffff00ff);
	vnDebugDraw::Line(&v[1], &v[3], 0xffff00ff);
	vnDebugDraw::Line(&v[2], &v[3], 0xffff00ff);
	vnDebugDraw::Line(&v[0], &v[2], 0xffff00ff);
}
